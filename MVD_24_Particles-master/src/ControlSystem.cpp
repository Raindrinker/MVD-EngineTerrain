...
void ControlSystem::setTerrain(ImageData terrain) {
	height_map = terrain;
}
...
void ControlSystem::update(float dt) {
	if (control_type == ControlTypeTerrain) {
		updateTerrain(dt);
	}
	else {
		updateFree(dt);
...
void ControlSystem::updateTerrain(float dt) {

	Camera& camera = ECS.getComponentInArray<Camera>(ECS.main_camera);
	Transform& transform = ECS.getComponentFromEntity<Transform>(camera.owner);

	//multiply speeds by delta time 
	float move_speed_dt = move_speed_ * dt * 0.3;
	float turn_speed_dt = turn_speed_ * dt;
	...
	//update camera position
	float imagex = ((camera.position.x / 0.4) + 250) * 1024 / 500;
	float imagey = ((camera.position.z / 0.4) + 250) * 1024 / 500;

	height[0] = 0;
	if (imagex >= 0 && imagex < 1024) {
		if (imagey >= 0 && imagey < 1024) {
			height_map.getPixel(imagex, 1024 - imagey, height);
		}
	}

	camera.position.y = height[0] / 255.0f * 20.0f + player_height;
