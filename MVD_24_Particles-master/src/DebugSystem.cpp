...
void DebugSystem::init(GraphicsSystem* gs, ControlSystem* cs) {
	graphics_system_ = gs;
	control_system_ = cs;
}
...
static float player_height = 2.0f;
...
ImGui::TreePop();
		...
		ImGui::SetNextWindowBgAlpha(1.0);
		ImGui::Begin("Custom", &show_imGUI_);


		if (ImGui::Button("Change camera mode")) {
			if (control_system_->control_type == ControlTypeTerrain) {
		...
		ImGui::SliderFloat("Player height", &(control_system_->player_height), 1.0f, 10.0f);
		...
		if(ImGui::Button("Spawn tree")){
			graphics_system_->publicSpawnTree();
		}
		if (ImGui::Button("Spawn forest")) {
			graphics_system_->publicSpawnForest();
		}
