...
void GraphicsSystem::spawnForest_(ImageData height_map, float step, float x, float y) {
	...
	for (int i = 0; i < 10; i++) {
		...
		spawnTree_(height_map, step, x2, y2);
		x2 += ((rand() % 100) / 100.0f - 0.5) * 20;
		y2 += ((rand() % 100) / 100.0f - 0.5) * 20;

	}

}

void GraphicsSystem::publicSpawnTree() {
	spawnTree_(height_map, 0.4, (float)(rand() % 1024), (float)(rand() % 1024));
}

void GraphicsSystem::publicSpawnForest() {
	spawnForest_(height_map, 0.4, (float)(rand() % 1024), (float)(rand() % 1024));
}

void GraphicsSystem::spawnTree_(ImageData height_map, float step, float x, float y) {

	int height[3];
	height_map.getPixel(x, 1024 - y, height);

	if (height[0] < 10) {
		return;
	}

	int ent_tree = ECS.createEntity("tree");
	Mesh& pmc = ECS.createComponentForEntity<Mesh>(ent_tree);

	int tree_geom;

	Shader* tree_shader = loadShader("data/shaders/trees.vert", "data/shaders/trees.frag");

	int mat_tree_index = createMaterial();
	Material& mat_tree = getMaterial(mat_tree_index);
	mat_tree.shader_id = tree_shader->program;


	pmc.material = mat_tree_index;
	pmc.render_mode = RenderModeForward;

	print("HEIGHT: " + to_string(height[0]));
	print("HEIGHT: " + to_string(height_map.width));

	float worldx = ((x * 500 / 1024) - 250) * step;
	float worldy = ((y * 500 / 1024) - 250) * step;

	if (height[0] > 64.0f) {
		tree_geom = createGeometryFromFile("data/assets/pine.obj");
	}
	else {
		tree_geom = createGeometryFromFile("data/assets/tree.obj");
	}

	pmc.geometry = tree_geom;


	Transform& t = ECS.createComponentForEntity<Transform>(ent_tree);
	t.makeTranslationMatrix(lm::vec3(worldx, height[0] / 255.0f * 20.0f, worldy));
